...
In ordered mode, a receiver can abort the broadcast so that receivers with a lower priority do not receive it.
Broadcast receivers allow applications to receive intents that are broadcast by the system or by other applications. They can register for system and application events, and when that event happens, the registered receivers get notified.
